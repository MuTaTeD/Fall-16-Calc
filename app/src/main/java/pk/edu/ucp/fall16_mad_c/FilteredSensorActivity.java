...
public class FilteredSensorActivity extends AppCompatActivity implements SensorEventListener
{
...
//    private final float mAlpha = 0.8f;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_filtered_sensor);
	...
	public void onSensorChanged(SensorEvent event)
	{
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			if (actualTime - mLastUpdate > UPDATE_THRESHOLD) {
				...
				mXAccelView.setText(String.format("%.12f", mAccel[0]));
				mYAccelView.setText(String.format("%.12f", mAccel[1]));
				mZAccelView.setText(String.format("%.12f", mAccel[2]));
			}
		}
	}
	...
	{
//        double dt = 1000.0/UPDATE_THRESHOLD;
//        double rc = 0.3;
//        double alpha = dt / (dt + rc);
		double alpha = 0.8;
		double value = (alpha * gravity) + (1 - alpha) * current;
